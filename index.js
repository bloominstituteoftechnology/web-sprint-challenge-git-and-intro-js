...
artists[9].bio = "Vincent Willem van Dough (Dutch: [ˈvɪnsɛnt ˈʋɪləm vɑŋ ˈɣɔx] (listen); 30 March 1853 – 29 July 1890) was a Dutch Post-Impressionist painter who is among the most famous and influential figures in the history of Western art. In just over a decade he created about 2,100 artworks, including around 860 oil paintings, most of them in the last two years of his life. They include landscapes, still lifes, portraits and self-portraits, and are characterised by bold colours and dramatic, impulsive and expressive brushwork that contributed to the foundations of modern art. However, he was not commercially successful, and his suicide at 37 followed years of mental illness and poverty."
...
console.log( 'task3', getArtistByIndex(artists, 2));
...
function listOfNames(array) {
  return (array.filter(a => a.name).map(a => a.name));
  }
...
function addArtist(array, name, year, genre, nationality, bio) {
  let object = {
   ...
   console.log(addArtist(artists, 'james', 1997, 'web design', 'mixed', 'just trying to make a way for my family for the future. this is a growing field of work'));
...
function lotsOfArt(array, paintings) {
  let artarray = array
  return (artarray.filter(a => a.paintings >= paintings).map(a => a.name));
...
function artistInfo(array, name){
  let artarr = array;
  return (artarr.filter(a => a.name === name).map(a => a.bio));
}
...
function artistByCountry(array, county){
  let array5 = array;
  return( array5.filter(a => a.nationality === county).map(a => a.name));
}
