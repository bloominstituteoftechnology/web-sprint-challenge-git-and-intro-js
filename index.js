...
function getArtistByIndex(array, index) {
  ...
  let anArtist = artists[index];

  return "The artist at index " + anArtist.id + " is " + anArtist.name + ".";
  }
...
function get20s(artists) {
  ...
  for (let i = 0; i < artists.length; i++) {
    let birthDeath = Number(artists[i].years.slice(0, 4)); 
    if (birthDeath >= 1900 && birthDeath <= 2000) {
      artistName.push(artists[i].name);
    }
  }
  return artistName;
...
function addArtist(id, name, years, genre, nationality, bio) {
    ...
    let newArtist = ({
      ...
      genre: "Web Design",
      nationality: "White",
      bio: "I am a Web Design student, looking forward to the journey ahead."
    })
