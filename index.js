...
function getArtistByIndex(ar,ind) {
  return(`the artist at index ${ar[ind].id} is ${ar[ind].name}`)
}
...
function listOfNames(ar){
  const artistName = []
  for(let i = 0;i<ar.length;i++){
    artistName.push(ar[i].name)
  }
  return(artistName)
}
...
function removeArtist(ar,nu) {
  const artistAlt = [...ar];
  artistAlt.splice(nu,1);
  return(artistAlt)
}
...
function addArtist(ar,na,ye,ge,nat,bio) {
  const newArray = [...ar];
  newArray.push({'name': na,'years':ye,'genre':ge,"nationality":nat,'bio':bio})
  return(newArray)
}
...
function artistByCountry(ar,nat){
  const artCountry = [];
  for(let i = 0;i<ar.length;i++){
    if(ar[i].nationality == nat){
      artCountry.push(ar[i].name)
    }
  }
  return(artCountry)
