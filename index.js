...
function get20s(nameArray){
  let i20s = []
  
  for (i = 0; i < artists.length; i++){
  let birthYear = parseInt(artists[i].years)
   if (birthYear >= 1900){ 
    i20s.push(artists[i])
   } 
  } 
...
myObject = [{id: "22", name: "triston wright", years: "1998 - current day", genre: "Web Design", nationality: "american", bio: "lorem ipsum lorem ipsum lorem ipsum"}]
function addArtist(nameArray,object){
  nameArray.push(object)
  return nameArray
  }
console.log(addArtist(artists,myObject))
...
let muchArt = []
function lotsOfArt(nameArray){
  for (i = 0; i < nameArray.length; i++){
    if (nameArray[i].paintings > 100){
      muchArt.push(nameArray[i].name)
    } 
  }console.log(muchArt)
}
