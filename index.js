...
function getArtistByIndex(artists, idx) {
  let artist = artists[idx].name;
  return `the artist at index ${idx} is ${artist}`
}
...
function get20s(data) {
  let century = function (str) {
    return str.match(/\d+/g)
  }
  console.log(century("1541 - 1614"))
  let arr = [];
  data.forEach((artist) => {
    let life = century(artist.years);
...
*/
function removeArtist(artist, idx) {
...
function addArtist(artists,) {
  let newArtist = {
    ...
    bio: "loves to code and tinker aroundd with tech"
  }
