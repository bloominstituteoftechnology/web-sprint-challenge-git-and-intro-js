...
}const { id, name } = artists[idx];return `the artist at index ${id} is ${name}`;
}
...
function artistInfo(artists, name){
  const filteredArtists = artists.filter(artist => artist.name && artist.name === name);
  if (filteredArtists.length > 0){
    return filteredArtists[0].bio;
  }
...
function artistByCountry(artists, nationality){
  const filteredArtists = artists.filter(artist => artist.nationality && artist.nationality.indexOf(nationality) !== -1);
  
  if (filteredArtists.length > 0) {
    return filteredArtists.map(artist => artist.name);
  }
