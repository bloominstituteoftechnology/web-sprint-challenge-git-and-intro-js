...
function get20s(artists){
  ...
  for ( i = 0; i < artists.length; i++){
    ...
    var lifetime = artist.years.split(" - ");
    var startCentury = Number(lifetime[0].substring(0, 2));
    var endCentury = Number(lifetime[1].substring(0, 2));

    if (startCentury == 19 || endCentury == 19){
      twentiesArr.push(artists[i]);
    }
    return twentiesArr
  }
  /* Code here */
...
function addArtist(arr, newId, newName, newYears, newGenre, newNationality, newBio){
arr.push({id: newId, name: newName, years: newYears, genre: newGenre, nationality: newNationality, bio: newBio});
    /* Code here */
return arr;
  }

  console.log(addArtist(artists, 21, "Micherre Fox", "December 19th, 1993 - Present", "Web Design", "American", "Family is from the Czech Republic but Micherre was born and raised in New York. She has a background in political science and Real Estate Lending."));
