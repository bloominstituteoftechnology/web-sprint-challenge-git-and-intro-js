...
let artistOneName = artists[0].name
console.log(artistOneName)

let artistThreeBio = artists[2].bio
console.log(artistThreeBio)


/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 2: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 
There is a typo in your dataset 😱 The 9th artist, Vincent Van Gogh is currently Vincent Van Dough. Use an array method to fix this issue and console.log() to check your work. */

function nameChange (arr, i, name)
{
  arr[i].name = name
  return arr
}

nameChange(artists, 8, "Vincent Van Gogh")
...
{
  const artistByIndex = array[i].name
  return `The artist at ${i} is ${artistByIndex}.`
}
...
{
 array[i].splice;
 return `${array[i].name} has been removed`;
}
...
function addArtist(obj, id, name, years, genre, nationality, bio){
  obj.id = id;
  obj.name = name;
  obj.years = years;
  obj.genre = genre;
  ...
  obj.bio = bio;
  return `You've added ${id} with the name of ${name}, born in ${years}..`
  }

  addArtist(artists, 20, 'pineapple', 1999, 'pop', 'purple', 'yes I did it')
...
function lotsOfArt(arr){
  ...
  for (var i = 0; i < arr.length; i++)
  {
    ...
    console.log( `This ${arr[i].name} has made more than 100 paintings! They've made ${arr[i].paintings}!`)
  } 
}
