...
function get20s(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    const aBy = parseInt(artist.years.substring(0.4));
    
    const aDy = parseInt(artist.years.substring(7));
    
    if (aBy > 1900 && aDy < 2000){
      c.push(arr[i].name);
    }
  }
  return c
...
function addArtist(arr) {
  let person = {
    ...
    years: "1898 - 1965",
    genre: "Web Design",
    nationality: "no Nationality Here",
    bio:
      "orem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to m",
  };
  arr.push(person);
  return arr;
}
