...
artists.splice = "Vincent Van Gogh";
...
*/
function getArtistByIndex(artists, artist.length) {
...
function get20s(artists , years, array.length){
  const time = [];
  for (let i = 0 ; i < artists.length; i++){
    if(artists[i].years < 2001)
  }and{
...
*/
function removeArtist(artists, array.length) {
  const artists = {
    removeThisProperty: true,
  };
delete artist.removeThisProperty;
...
function addArtist(artists, array lenght){
  const newArtist = [...artists];
  id.unshift(46);
  name.unshift("Bobby Lane");
  genre.unshift("Web Design");
  nationality.unshift("USA");
  bio.unshift("lorem ipsum");
  }
...
function lotsOfArt(artists, array.length){
  const overHundred = [i]
  for (let i = 0 ; i < artists.length; i++){
    if(artists[i].paintings > 100);
    console.log(overHundred0;
  }
...
function randomize(1, 2, 3 4, 5, 6){
