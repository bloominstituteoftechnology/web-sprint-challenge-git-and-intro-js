...
console.log(artists[0].name); // why can i not use [2] instead of .name?

//(2) Bio of the third artist (2nd index) in the array 
console.log(artists[2].bio); // same with [6] for .bio?
...
// function getArtistByIndex(anyArt,artIndex) { //receive an array and a number
//   for(let artIndex = 0; artIndex < anyArt.length; artIndex++){
//     return `The artist at index ${artIndex} is ${[artIndex.name]}`;
//   }
// }  
// console.log(getArtistByIndex(1));
...
function removeArtist(artArr){
   artArr.splice([artIndex]); // something to do with .length-1
   console.log(artArr);
}
...
2. Add this object of information to the end of the array
    { 
      ...
      genre: Web Design, 
      nationality: Yur Nationality Here
...
function addArtist(artists, id, name, years, genre, nationality, bio){ // i know theres a cleaner way to do this, moving on for now
...
// .filter the painting key in artist'
function lotsOfArt(artistArr){
  const artOver100 = artistArr.filter(artistArr => artistArr[8].length > 100);
  return artOver100;
}
