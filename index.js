...
function get20s(arrayRecieved) {
  let newArray = [];
  for (let artist of arrayRecieved) {
...
function lotsOfArt(arrayRecieved) {
  let newArray2 = [];
  for (let i = 0; i < arrayRecieved.length; i++) {
    if (arrayRecieved[i].paintings > 100) {
      newArray2.push(arrayRecieved[i].name);
    }
  }
  return newArray2;
}
...
// _______________ SHORTER WAY BELOW! _____________________

// function lotsOfArt(arr) {
//   return artists.filter(paint => paint.paintings > 100).map(name => name.name);
// }
