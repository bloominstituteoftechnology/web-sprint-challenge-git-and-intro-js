...
artists.map((artist) => {
  ...
  if (artist.name === "Vincent van Dough") {
    console.log("Before: ", artist.name);
    artist.name = "Vincent van Gogh";
    console.log("After: ", artist.name);
  }
});
/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 3: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 
...
function get20s(artists) {
  const newArr = [];
  artists.map((artist) => {
    const years_arr = artist.years.split(" - ");
    //console.log(years_arr)
    const birthVar = parseInt(years_arr[0]);
    const deathVar = parseInt(years_arr[1]);
    //console.log(birthVar);
    if (1900 <= birthVar && deathVar < 2000) {
...
function addArtist(artistsArray, newArtist) {
  artistsArray.push({ id: artistsArray.length + 1, ...newArtist });
  return artistsArray;
}
...
function lotsOfArt(artists) {
  const artistsArray = artists.filter((artist) => artist.paintings > 100);
