...
console.log("The first artist name is " + artists[0].name);

//(2) Bio of the third artist (2nd index) in the array 

console.log("The third artist bio is " + artists[2].bio);
...
function getArtistByIndex(artists, artistIntro) {
  for(var i = 0; i <= artists.length; i++){
    artistIntro = console.log("The artist at index " + artists[i].id + "is " + artists[i].name );
  } 
  return artistIntro;
}
getArtistByIndex();
...
function removeArtist(artists, artistGottaGo) {
  let newArray = [];
  newArray.push(artists);
  artistGottaGo = newArray[4];
  newArray.delete(artistGottaGo);
...
function addArtist(artists, name, years, genre, nationality, bio) {
  ...
  "bio" : "I have worked with Publix Supermarkets for the past 9 years, and I'm ready for a change in workflow and tax bracket. Also would be great to spend more time with my wife and kids."
  }

  let meArray = [];

  meArray.push(artists);
...
function artistInfo(artists, artistInfo){
  artistInfo = artists[13].bio;
...
function artistByCountry(artists, nationality){
  let nationalArr = [];
  for(i = 0; i <= artists.length; i++){
    if(artists[i].nationality === "French"){
      nationalArr.push(artists[i]);
    }
  }
  return nationalArr;
}
