...
function get20s(array){
  ...
  for (let i = 0; i < array.length; i++){
    const newVar = array[i]['years'].split(' ');
    if((newVar[0] >= 1900) && (newVar[2] <= 2000)){
...
array.push({
  id: 20,
  name: 'Michael Cavellier', 
  years: '1999 - current day',
  genre: 'Web Design', 
  nationality: 'Caucasian',
  bio: 'Born in georgia.  Is hungry and wants to complete this sprint because he has not eaten :).'
})
return array
}
...
function lotsOfArt(array){
  ...
  for(let i = 0; i < array.length; i++){
    if(artists[i].paintings >= 100){
      artistSelector.push(array[i].name)
    }
  }
