...
function get20s(array){
      let array20s = [] //initialize return array
  for (let i in array){ // go through each object in the array

   let birthDeath = array[i].years.split(" - ") // get birth years and death years into an array
  
    if (birthDeath[0]>=1900 && birthDeath[1]<=2000){ //check that birth and death are both within 20th century
      array20s.push(array[i].name) //if so, put name of artist into an array

    }
  }
  return array20s //return the array
}
...
function addArtist(array){
    const artistObject = {
      ...
      genre: "Web Design", 
      nationality: "USA",
      bio: "Trained in civil engineering and construction in his late teens and early 20s. At 27, he decided to shift into the world of web development and never looked back." 
    }
...
function lotsOfArt(array){
  const muchArtArray = [] // initialize array to put artists w a lot of art
  for (let i in array){ // evaluate each artist object
    if (array[i].paintings > 100){ // if painting count is more than 100
      muchArtArray.push(array[i].name) // take artist name and put it in the muchArtArray to be returned at end
