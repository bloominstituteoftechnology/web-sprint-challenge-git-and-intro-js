...
function getArtistByIndex(identity){
  console.log(`the artist at index ${artists[identity].id} is ${artists[identity].name}`);
}
...
function get20s(){
  console.log(artists.filter(years => years.includes = "1900-2000"));
 }
 get20s();
...
function addArtist(object){
  object.push({ 
      ...
      bio: "Talented individual, hard worker",
    }  );
}
...
function lotsOfArt(){
  console.log(artists.filter(paintings => paintings.includes >= 100));
}
