...
let artist0 = artists.indexOf([0, 2]);
let artist1 = artists.indexOf([2, 5]);
...
function getArtistByIndex(arr, id) {
  var artistObj = arr[id]; // select array index
  var artistName = artistObj.name; //select object
  return "the artist at index " + id + " " + "is " + artistName; //concatenation 
}
...
function get20s(array) {
  let ageRange = []; //create array
  array.forEach((artist) => { // select artist element using forEach method
    let birthYears = artist.years.split(" - "); // removes spaces and - from years
    let birthYear = Number(birthYears[0]); // turns string to number
    let deathYear = Number(birthYears[1]);// turns string to number
    if (birthYear >= 1900 && deathYear <= 2000) { //set condition
      ageRange.push(artist.name); // add name to array if it falls within range
    }
  });
  return ageRange;
}
...
*/
function removeArtist(/*Your Code Here*/ arr, id) {
  /* code here */

  let data = arr; // create an array
  data.splice(id, 1); // splices the data (id=position, 1=how many to remove)
  return data.length;
}
...
/*
  function argument (array, index)
  array specifies which array to use
  index specifies which elements in the array to removeArtist
...
function addArtist(/*Your Code Here*/ arr, obj) {
  /*Your Code Here*/
  let data = arr;
  data.push(obj);
  return data;
...
function lotsOfArt(/*Your Code Here*/ arr) {
  /*Your Code Here*/
  let veryActivePainters = [];
  arr.forEach((artists) => {
    let totalPaintings = Number(artists.paintings);
    if (totalPaintings > 100) {
      veryActivePainters.push(artists.name);
    }
  });

  return veryActivePainters;
}
