 ...
 */
/*function getArtistByIndex(name, index) {
  return `The artist at index ${index} is {name}.`;
}
console.log(getArtistByIndex(artists["name"], 0));*/

function getArtistByIndex(array, index) {
  return (
    "The artist at index " + array[index].id + " is " + array[index].name + "."
  );
}
...
/*function lotsOfArt(array) {
  ...
  var x = 0;
  for (var obj of array) {
    if (obj.paintings.startsWith(>100)) {
      temp[x] = obj.name;
      x++;
    }
  }
