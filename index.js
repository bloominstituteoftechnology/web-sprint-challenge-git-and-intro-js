...
function getArtistByIndex(artArr, indNum) {
	return `the artist at index ${indNum} is ${artArr[indNum].name}`;
}
...
function get20s(artArr) {
	...
	for (let i = 0; i < artArr.length; i++) {
		if (1900 <= artArr[i].years.split("-")[0]) {
			twenCent.push(artArr[i].name);
		}
	}
	return twenCent;
}
...
function removeArtist(artArr, indexNum) {
	artArr.splice(indexNum, 1);
